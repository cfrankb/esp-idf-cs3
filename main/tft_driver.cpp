...
void lcdInit(TFT_t *dev, uint16_t model, int width, int height, int offsetx, int offsety)
{
    ...
    {
        init7796(dev, width, height); // 0x7796
    }

    if (dev->_model == 0x9340 || dev->_model == 0x9341 || dev->_model == 0x7735)
    {
        // init9341(dev, width, height); // endif 0x9340/0x9341/0x7735
        init9341v2(dev, width, height); // endif 0x9340/0x9341/0x7735
    }

    if (dev->_model == 0x7789)
    {
        init7789(dev, width, height); // 0x7789
    }

    if (dev->_model == 0x9225)
    {
        init9225(dev, width, height); // endif 0x9225
    }

    if (dev->_model == 0x9226)
    {
        init9226(dev, width, height);
    }
...
void init9341v2(TFT_t *dev, int &width, int &height)
{
    ...
    spi_master_write_comm_byte(dev, 0xEA); // EXTRA
    spi_master_write_data_byte(dev, 0x00);
    spi_master_write_data_byte(dev, 0x00);
    ...
    spi_master_write_comm_byte(dev, 0xC1); // Power Control 2											EXTRA
    spi_master_write_data_byte(dev, 0x00);

    spi_master_write_comm_byte(dev, 0xC5); // VCOM Control 1											EXTRA
    spi_master_write_data_byte(dev, 0x30);
    spi_master_write_data_byte(dev, 0x30);

    spi_master_write_comm_byte(dev, 0xC7); // VCOM Control 2											EXTRA
    spi_master_write_data_byte(dev, 0xB7); // -- 0xB7 -- 0x86

    spi_master_write_comm_byte(dev, 0x3A); // Pixel Format Set
    spi_master_write_data_byte(dev, 0x55); // 65K color: 16-bit/pixel

    spi_master_write_comm_byte(dev, 0x36); // Memory Access Control
    spi_master_write_data_byte(dev, 0x48); // Right top start, BGR color filter panel 		EXTRA
    // spi_master_write_data_byte(dev, 0x00);//Right top start, RGB color filter panel

    spi_master_write_comm_byte(dev, 0x21); // Display Inversion OFF		0x20	ON 0x21		###.

    spi_master_write_comm_byte(dev, 0xB1); // Frame Rate Control						EXTRA
    spi_master_write_data_byte(dev, 0x00);
    spi_master_write_data_byte(dev, 0x1A);

    spi_master_write_comm_byte(dev, 0xB6); // Display Function Control 			EXTRA
    // spi_master_write_data_byte(dev, 0x0A);	// DFRobot
    // spi_master_write_data_byte(dev, 0xA2);	// DFRobot
    spi_master_write_data_byte(dev, 0x08);
    spi_master_write_data_byte(dev, 0x82); // REV:1 GS:0 SS:0 SM:0
    spi_master_write_data_byte(dev, 0x27);
    // spi_master_write_data_byte(dev, 0x00);

    spi_master_write_comm_byte(dev, 0xF2); // 3Gamma Function Disable   		EXTRA
    spi_master_write_data_byte(dev, 0x00);

    spi_master_write_comm_byte(dev, 0x26); // Gamma CURVE SELECTED
    spi_master_write_data_byte(dev, 0x01);

    spi_master_write_comm_byte(dev, 0xE0); // Positive Gamma Correction			EXTRA
    spi_master_write_data_byte(dev, 0x0F);
    ...
    spi_master_write_data_byte(dev, 0x43);
    ...
    spi_master_write_comm_byte(dev, 0xE1); // Negative Gamma Correction			EXTRA
    spi_master_write_data_byte(dev, 0x00);
    ...
    spi_master_write_data_byte(dev, 0x2B);
    spi_master_write_data_byte(dev, 0x56);
    ...
    spi_master_write_comm_byte(dev, 0x2B); // DFRobot												// EXTRA
    spi_master_write_data_byte(dev, 0x00);
    spi_master_write_data_byte(dev, 0x00);
    spi_master_write_data_byte(dev, 0x01);
    spi_master_write_data_byte(dev, 0x3f);

    spi_master_write_comm_byte(dev, 0x2A); // DFRobot												// EXTRA
    spi_master_write_data_byte(dev, 0x00);
    spi_master_write_data_byte(dev, 0x00);
    spi_master_write_data_byte(dev, 0x00);
